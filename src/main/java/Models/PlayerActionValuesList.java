...
public class PlayerActionValuesList {
    ...
    private void fill(PlayerAction playerAction, GameState gameState, GameObject bot, Teleporter myTeleporter) {
        ...
        for (var player : otherPlayerList) {
            values.add(new PlayerActionValues(bot, playerAction, PlayerActions.FIRETORPEDOES,
                    getHeadingBetween(bot, player)));
        ...
        if (bot.getTeleporterCount() > 0 && bot.getSize() > 45 && myTeleporter.isTeleporterActive() == false) {
            for (var player : otherPlayerList) {
                if (bot.getSize() - 20 > player.getSize()) {
                    values.add(new PlayerActionValues(bot, playerAction, PlayerActions.FIRETELEPORT,
                            getHeadingBetween(bot, player)));
                    values.get(values.size() - 1).setTarget(player);
                }
            }
        ...
        if (myTeleporter.isTeleporterActive()) {
            var teleporterList = gameState.getGameObjects().stream()
                    .filter(item -> item.getGameObjectType() == ObjectTypes.TELEPORTER).collect(Collectors.toList());
            ...
            for (var teleporter : teleporterList) {
                if (teleporter.getId().equals(myTeleporter.getId())) {
                    ...
                    if (dist <= 10) {
                        System.out.println("Teleporter target position: (" + myTeleporter.getTargetPosition().getX()
                                + ", " + myTeleporter.getTargetPosition().getY() + ")");
                        System.out.println("Teleporter position: (" + teleporter.getPosition().getX() + ", "
                                + teleporter.getPosition().getY() + ")");
                        GameObject target = new GameObject(null, null, null, null, teleporter.getPosition(), null, null,
                                null, null, null, null);
                        ...
                        double distBotToTarget = getDistanceBetween(bot.getPosition(),
                                myTeleporter.getTargetPosition());

                        if (distBotToTeleporter > distBotToTarget) {
                            // Teleporter telah melewati target
                            System.out.println("Teleporter target position (kelewatan): ("
                                    + myTeleporter.getTargetPosition().getX() + ", "
                                    + myTeleporter.getTargetPosition().getY() + ")");
                            System.out.println("Teleporter position (kelewatan): (" + teleporter.getPosition().getX()
                                    + ", " + teleporter.getPosition().getY() + ")");

                            GameObject target = new GameObject(null, null, null, null, teleporter.getPosition(), null,
                                    null, null, null, null, null);
                            values.add(new PlayerActionValues(bot, playerAction, PlayerActions.TELEPORT, 0));
                            values.get(values.size() - 1).setTarget(target);
                        } else {
                            // Teleporter belum melewati target
                            System.out.println("Teleporter target position (belum kelewatan): ("
                                    + myTeleporter.getTargetPosition().getX() + ", "
                                    + myTeleporter.getTargetPosition().getY() + ")");
                            System.out.println("Teleporter position (belum kelewatan): ("
                                    + teleporter.getPosition().getX() + ", " + teleporter.getPosition().getY() + ")");

                            for (var player : otherPlayerList) {
                                if (bot.getSize() > player.getSize()
                                        && getDistanceBetween(teleporter, player) <= player.getSize() + bot.getSize()) {
                                    GameObject target = new GameObject(null, null, null, null, teleporter.getPosition(),
